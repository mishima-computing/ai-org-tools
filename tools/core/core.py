#!/usr/bin/env python3
"""core — the medium-agnostic engine shared by every media division (graphicist, composer, …).

core knows nothing about graphics: the produce ladder, the Library, GATE/JUDGE and capture-as-asset are
generic; a division plugs in its own providers (the revolver chambers) and brief vocabulary.

Two layers (the structure): the SOURCE (held assets, or a structured model built with Blender/rules) is
reusable and grounded; the VIEW (the flat image) is a disposable projection of it. PIL, gen-AI, Blender,
the asset library are NOT distinct architecture — they are interchangeable PROVIDERS, the chambers of a
revolver the tool rotates through. The controller calls produce() DIRECTLY (a workflow call, NOT via MCP;
see docs/decisions/ADR-0001).

The asset-first ladder (descend only when the rung above can't cover the brief):
  1 reuse-apply    — SOURCE: a held asset matches as-is                              (GATE)
  2 reuse-process  — SOURCE: edit a held asset to fit (restyle/recolor/retopo)       (GATE)
  3 build+render   — SOURCE built from rules + VIEW rendered explicitly (Blender/PIL) (GATE)
  4 synthesize     — VIEW synthesized by gen-AI, conditioned on the brief; last resort (JUDGE)

Rungs 1-3 are EXPLICIT (you specify it -> reproducible, auditable -> GATE); rung 4 is SYNTHESIZED (a
model supplies unspecified content from priors -> JUDGE: aesthetic + brief fidelity + a grounding audit
of WHY no asset covered it). Note rungs 3 and 4 are BOTH generation — they differ only in explicit vs
synthesized (a verification difference), not in structure. Any output is captured back into the library,
so the gap closes and synthesis is used less over time. The same shape produces a music tool (source =
MIDI/score/samples; view = audio via synth (GATE) or gen-audio (JUDGE)) — see ADR-0001.
"""
...
def _self_test():
    ...
    print("core self-test ok: asset-first ladder, generation last + captured, self-shrinking")
